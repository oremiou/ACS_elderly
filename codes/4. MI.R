...
d1=d0[d0$Outcome=="myocardialinf",]
...
CompoFE <- ggplot(data = hist.y, aes(x = x, ymin = 0, ymax = y, fill = area2)) +
  ...
  annotate(geom = 'text', x = 1.03, y = 0.2, color = 'Black', label = '0.04%', hjust = -0.1)+
  annotate(geom = 'text', x = 0.70, y = 0.2, color = 'Black', label = '99.96%', hjust = -0.1)+
  labs(fill = "")+
  labs(y= "Posterior probability density", x = "Hazard ratio")+
  labs(title = "Myocardial Infarction")+
  theme(
