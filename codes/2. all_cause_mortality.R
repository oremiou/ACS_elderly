...
d1=d0[d0$Outcome=="allcausemortality",]
...
CompoFE <- ggplot(data = hist.y, aes(x = x, ymin = 0, ymax = y, fill = area2)) +
  ...
  annotate(geom = 'text', x = 1.03, y = 0.2, color = 'Black', label = '91%', hjust = -0.1)+
  annotate(geom = 'text', x = 0.92, y = 0.2, color = 'Black', label = '9%', hjust = -0.1)+
  labs(fill = "")+
